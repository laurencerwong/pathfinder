    ...
    else if(currentTutorialStep == 1 && currentlySelectedCharacter_ != nil)
    {
        if([currentlySelectedCharacter_ hasAlreadyMoved])
        {
            ...
            CCSequence *runStep3 = [CCSequence actions:
                                    [CCDelayTime actionWithDuration:7.0],
...
-(void) runTutorialStepZero
{
    ...
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"WELCOME TO THE TUTORIAL! Touch a character to begin." originatingFrom:ccp(s.width/2, s.height/2) withTimeout:7.0];
}

-(void) runTutorialStepOne
{
    QuietLog(@"Running Tutorial Step 1");
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"Touch and drag any player on your team to set a path for them. Watch your energy for each path on the blue bar." originatingFrom:currentlySelectedCharacter_.position withTimeout:7.0];
}

-(void) runTutorialStepTwo
{
    QuietLog(@"Running Tutorial Step 2");
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"Thank for moving me!\nWhen both players have ended their turns, I will move along the path you drew" originatingFrom:currentlySelectedCharacter_.position withTimeout:7.0];
}

-(void) runTutorialStepThree
{
    QuietLog(@"Running Tutorial Step 3");
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"Touch the Combat button with the sword to make this a combat move" originatingFrom:currentlySelectedCharacter_.position withTimeout:7.0];
}

-(void) runTutorialStepFour
{
    QuietLog(@"Running Tutorial Step 4");
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"Combat moves mean your player will stop and fight if he collides with an enemy" originatingFrom: ccp(400,30) withTimeout:7.0];
}

-(void) runTutorialStepFive
{
    QuietLog(@"Running Tutorial Step 5");
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"Touch the NonCombat button to change your combat move to a non-combat path" originatingFrom: ccp(370,35) withTimeout:7.0];
}

-(void) runTutorialStepSix
{
    QuietLog(@"Running Tutorial Step 6");
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"Touch the Undo button to redraw your path" originatingFrom: ccp(430,35) withTimeout:7.0];
}

-(void) runTutorialStepSeven
{
    QuietLog(@"Running Tutorial Step 6");
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"Redraw the path for your character and the paths for the rest of your characters before hitting 'End Turn'" originatingFrom:currentlySelectedCharacter_.position withTimeout:7.0];
}
...
-(void)setCameraPositionTo:(CGPoint)point
{
    self.camera.desiredPosition = [self checkBounds:point];
}
...
-(int) checkList:(NSArray*)list ForNodeWithState:(CGPoint) position
{
    for(int i = 0; i < list.count; i++) {
        Node *n = [list objectAtIndex:i];
...
-(void) draw
{
    ...
    if(self.currentlySelectedCharacter != nil) {
        ccDrawColor4B(120, 0, 255, 255);
        glLineWidth(8);
        //ccDrawCircle(self.currentlySelectedCharacter.position, self.currentlySelectedCharacter.currentEquipment.range, CC_DEGREES_TO_RADIANS(360), 64, NO);
        ccDrawCircle(lastTouchMovedLocation, self.currentlySelectedCharacter.currentEquipment.range, CC_DEGREES_TO_RADIANS(360), 64, NO);
    }
    ...
    if(debugDraw)
    {
        ...
        //
        //                if(self.open != nil) {
        //                    int index = [self checkList:open_ ForNodeWithState:ccp(i, j)];
        //                    if(index != -1) {
        //                        ccDrawColor4B(0, 0, 255, 255);
        //                        ccDrawPoint(ccp(i*tilesize, (mapHeight-j)*tilesize));
        //                    }
        //                }
        //
        //                if(self.path != nil) {
        //                    int index = [self checkList:path_ ForNodeWithState:ccp(i, j)];
        //                    if(index != -1) {
        //                        ccDrawColor4B(0, 255, 0, 255);
        //                        ccDrawPoint(ccp(i*tilesize, j*tilesize));
        //                    }
        //                }
        //
        //                if(self.closed != nil) {
        //                    int index = [self checkList:closed_ ForNodeWithState:ccp(i, j)];
        //                    if(index != -1) {
        //                        ccDrawColor4B(255, 95, 0, 255);
        //                        ccDrawPoint(ccp(i*tilesize, j*tilesize));
        //                    }
        //                }
        //            }
...
-(BOOL) ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayNextPersistentMessage];
