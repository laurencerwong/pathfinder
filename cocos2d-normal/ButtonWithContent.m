...
{
    return [[self alloc] initWithNoContent:normalImage SelectedImage:selectedImage Target:target Selector:selector];
}

-(id)initWithNoContent:(NSString *)normalImage SelectedImage:(NSString *)selectedImage Target:(id)target Selector:(SEL)selector
{
    self = [super initWithNormalImage:normalImage selectedImage:selectedImage disabledImage:normalImage target:target selector:selector];
    if (self)
    {
