...
-(id) init
{
	if( (self=[super init]) ) {
        ...
        [self initializeFastTraversableGrid];
    }
    return self;
}
...
//        tileMap_ = [CCTMXTiledMap tiledMapWithTMXFile:@"bigtiledesert.tmx"];
        tileMap_ = [CCTMXTiledMap tiledMapWithTMXFile:@"small_map_layers.tmx"];
    }
...
{
//    QuietLog(@"Checking collidable slow...");
    BOOL collideable = NO;
    if([self isValidTileCoord:tileCoord])
    {
...
-(void) initializeFastTraversableGrid
{
    if(self.tileMap == nil) {
        QuietLog(@"TileMap is nil, cannot initialize fast traversable grid");
        return;
    }
    
    int mapWidth = self.tileMap.mapSize.width;
    int mapHeight = self.tileMap.mapSize.height;
    
    //Set up grid
    traversableGrid = malloc(mapWidth * sizeof(int*));
    for(int i = 0; i < mapWidth; i++) {
        traversableGrid[i] = malloc(mapHeight * sizeof(int));
    }
    
    for(int i = 0; i < mapWidth; i++) {
        for(int j = 0; j < mapHeight; j++) {
            traversableGrid[i][j] = ( [self isTileCollidable:ccp(i, j)] ? 1 : 0);   //set grid to be 1 if space was collideable
        }
    }
}
...
{
//    QuietLog(@"Checking collidable fast...");
    return traversableGrid[(int)tileCoord.x][(int)tileCoord.y] == 1;
}
