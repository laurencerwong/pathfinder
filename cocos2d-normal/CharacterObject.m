...
//AI
@synthesize currentTarget = currentTarget_;
@synthesize behaviors = behaviors_;
...
-(id)initializeWithFile:(NSString *)filename AndPlayerID:(int)inPlayerID AndColor:(ccColor4F)color
{
    ...
    if (self) {
        ...
        behaviors_ = [[NSMutableArray alloc] init];
...
{
    float distSqFromCenterToCenter = ccpLengthSQ(ccpSub([self position], [node position]));
    float rangeSq = powf([self innerHitCircleRadius], 2);
    if(distSqFromCenterToCenter < rangeSq)
    {
        return YES;
    }
    return NO;
...
-(int) getTotalAttack
{
    return self.stats.attack + self.currentEquipment.stats.attack;
}

-(int) getTotalDefense
{
    return self.stats.defense + self.currentEquipment.stats.defense;
}
...
-(void)takeDamage:(int)damage
{
    ...
    if(self.currentHealth <= 0) {
        [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler]
         displayBloodExplosionAtPosition:self.position];
        if(!self.hasFinishedMoving)
        {
            [self tellSimulatorFinishedMoving];
        }
        self.isDead = YES;
    }
}
...
{
    float damage = self.stats.attack + self.currentEquipment.stats.attack + arc4random() % 5;
    QuietLog(@"Character %d entered combat", self.actionerID);
    
    if([self makeAccuracyCheckWithPlayer:character])  //Did I hit him?
    {
        [self doAttackAnimation];
        switch(currentEquipment_.type)
        {
            case SwordType:
                [character takeDamage:damage];
                [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displaySlashAtPosition:character.position];
                break;
            case LanceType:
                [character takeDamage:damage];
                [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displaySlashAtPosition:character.position];
                break;
            case BowType:
                [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] shootArrowFromCharacter:self toPoint:character.position withDamage:(int)damage];
                break;
            case WizardStaffType:
                [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler]
                 shootMagicFromCharacter:self toPoint:character.position withDamage:(int)damage];
                break;
        }
        QuietLog(@"Character %d dealt %d damage to character %d", [self actionerID], (int)damage, [character actionerID]);
    }
    else
    {
        [[BatchBattleSpriteHandler sharedBatchBattleSpriteHandler] displayTimedMessage:@"Missed" originatingFrom:self.position withTimeout:3.0];
        QuietLog(@"Character %d missed Character %d", self.actionerID, character.actionerID);
    }
