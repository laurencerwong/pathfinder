...
{
        NSString *text = [NSString stringWithFormat:@"Player %d Wins", playerNumber + 1];
    victoryLabel =[CCLabelTTF labelWithString:text fontName:@"Marker Felt" fontSize:30];
    CGSize s = [[CCDirector sharedDirector] winSize];
    victoryLabel.position = ccp(-70, s.height/2);
    [self addChild:victoryLabel];
    [victoryLabel runAction:[CCEaseOut actionWithAction:[CCMoveBy actionWithDuration:0.4f position:ccp(s.width/2 + 70,0)] rate:5]];
}
...
-(void) initButtons
{
    ...
    ButtonWithContent *moveButton = [ButtonWithContent ButtonWithIcon:@"MoveIcon.png" NormalImage:@"unpressedMenuButton.png" SelectedImage:@"pressedMenuButton.png" Target:self Selector:@selector(buttonPressed:)];
    moveButton.tag = MoveButton;
    [self pushMenuItem:moveButton];
}
